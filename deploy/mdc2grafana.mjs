 ...
 *
 * Usage: node deploy/mdc2grafana.mjs <project-dir|project.mdc.json> [-o dash.json]
 */
import { writeFile } from "node:fs/promises";
import { loadProject, iterMessages } from "../Embedded-Sharepoint/can/mdc/lib/mdc-load.mjs";
...
if (!projectPath) {
  console.error("Usage: node deploy/mdc2grafana.mjs <project-dir|project.mdc.json> [-o dash.json]");
  process.exit(2);
}
...
function fieldConfig(sig) {
  ...
  const min = d.scaleMin ?? sig.minimum;
  const max = d.scaleMax ?? sig.maximum;
...
const titleName = project.name ?? project.metadata?.name;
const dashboard = {
  title: titleName ? `${titleName} — Telemetry` : "MDC Telemetry",
  schemaVersion: 39,
  editable: true,
