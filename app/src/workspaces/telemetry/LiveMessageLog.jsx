...
import { formatTime, formatValue3 } from '../shared/formatters.js';
...
export default function LiveMessageLog() {
  ...
  const checkAtBottom = useCallback(() => {
    ...
    return st + clientHeight >= scrollHeight - SCROLL_THRESHOLD;
  }, []);
  ...
  const togglePause = useCallback(() => {
    setPaused((p) => {
      ...
      if (!next) pendingMsgsRef.current = [];
      return next;
    });
  }, []);
            ...
            return (
              ...
              >
                ...
                <Group gap={6} wrap="nowrap">
                  <Text size="sm" style={{ color: msg.message_name != null ? 'var(--text)' : 'var(--status-red)' }}>
                    {msg.can_id_hex}
                    {msg.message_name != null ? ` · ${msg.message_name}` : ' · Not Found'}
                  </Text>
                  {msg.sender && (
                    <Text size="xs" c="dimmed" style={{ opacity: 0.5, flexShrink: 0 }}>
                      {msg.sender}
                    </Text>
                  )}
                </Group>
                <Collapse in={isExpanded}>
                  <Stack gap={4} mt="xs" pl="xs" style={{ borderLeft: '2px solid var(--border)' }}>
                    ...
                    {msg.raw_packet && (
                      <Text size="xs" c="dimmed" style={{ fontFamily: 'var(--font-mono)', opacity: 0.6 }}>
                        {msg.raw_packet}
                      </Text>
                    )}
                  </Stack>
