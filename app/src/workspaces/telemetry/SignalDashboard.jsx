...
import { useLiveMessages } from '../../lib/useEngine.jsx';
import { formatTime, formatValue3, isIndexSignalName } from '../shared/formatters.js';
import { mergeBatchIntoCache, parseCacheKey } from '../shared/signalCache.js';

const UI_FLUSH_INTERVAL_MS = 80;
const LS_CAN_KEYS = 'can_signal_dashboard_v1_can_keys';
const LS_ECUS = 'can_signal_dashboard_v1_ecus';
const LS_SEARCH = 'can_signal_dashboard_v1_search';
...
export default function SignalDashboard() {
  ...
  const mergeIntoCache = useCallback((msgs) => {
    ...
    setCache((prev) => mergeBatchIntoCache(prev, msgs));
  }, []);

  useLiveMessages((batch) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (merged.length > 0) mergeIntoCache(merged);
  ...
  const idOptions = useMemo(() => {
    const list = allEntries.map((e) => ({ key: e.idKey, canId: e.canId, ecu: e.sender }));
  ...
  useEffect(() => {
    ...
    setSelectedIdKeys((prev) => {
      if (prev === undefined) return allIds;
  ...
  useEffect(() => {
    if (ecuOptions.length === 0) return;
    const allowed = new Set(ecuOptions);
    setSelectedEcus((prev) => {
      if (prev === undefined) return [...ecuOptions];
  ...
  const selectedIdsSet = useMemo(() => {
    if (selectedIdKeys === undefined) return new Set(idOptions.map((o) => o.key));
    return new Set(selectedIdKeys);
  }, [selectedIdKeys, idOptions]);

  const selectedEcusSet = useMemo(() => {
    if (selectedEcus === undefined) return new Set(ecuOptions);
  ...
  return (
    ...
    >
      <Group gap="md" mb="lg" align="center">
        <Text size="md" fw={600} style={{ color: 'var(--text)' }}>
          Signal Dashboard
        </Text>
        <ActionIcon
          variant="subtle"
          ...
          onClick={() => {
            ...
            if (!next) pendingBatchesRef.current = [];
          }}
        >
            ...
            <ScrollArea style={{ flex: 1 }} type="auto" scrollbarSize={8}>
              <Box p="sm">
                    ...
                    <Checkbox
                      ...
                      onChange={(e) => {
                        ...
                        setSelectedIdKeys((prev) => {
                          ...
                          if (checked) return base.includes(o.key) ? base : [...base, o.key];
                ...
                <Group justify="space-between" mb={6}>
                  ...
                  <Group gap={4}>
                    <Button variant="subtle" size="compact-xs" onClick={() => setSelectedEcus(ecuOptions)}>
                      Select all
                    </Button>
                    <Button variant="subtle" size="compact-xs" color="gray" onClick={() => setSelectedEcus([])}>
                      Deselect all
                    </Button>
                  </Group>
                </Group>
                    ...
                    <Checkbox
                      ...
                      onChange={(e) => {
                        ...
                        setSelectedEcus((prev) => {
                          ...
                          if (checked) return base.includes(ecu) ? base : [...base, ecu];
                        ...
                        return (
                          ...
                          >
                            <Group gap="xs" justify="space-between" wrap="nowrap">
                              <Text size="sm" style={{ color: notFound ? 'var(--status-red)' : 'var(--text)' }}>
                                {canId}
                            ...
                            <Stack gap={4} mt="xs" pl="xs" style={{ borderLeft: '2px solid var(--border)' }}>
                              {hasSignals &&
                                Object.entries(msg.signals).map(([name, value]) => {
                                  ...
                                  if (msg.is_array && isIndexSignalName(name)) return null;
