...
import { useEngineStatus, useLiveMessages } from '../../lib/useEngine.jsx';
import { engineClient } from '../../lib/engineClient.js';

const { apiJson } = engineClient;
import { mergePivotByFrameSignal } from './mergeByFrame.js';
import { SimpleLineChart } from './SimpleLineChart.jsx';

const LS_KEY = 'can_analytics_views_v1';
const ANALYTICS_TIME_RANGE = '-1h';
const LIVE_THROTTLE_MS = 280;
...
function sortSignalsByStartBit(signals) {
  ...
  return [...signals].sort((a, b) => {
    const sa = a.start_bit != null && !Number.isNaN(Number(a.start_bit)) ? Number(a.start_bit) : 1e9;
    const sb = b.start_bit != null && !Number.isNaN(Number(b.start_bit)) ? Number(b.start_bit) : 1e9;
...
export default function AnalyticsWorkspace() {
  ...
  useEffect(() => {
    if (status?.vehicle) setVehicleHint(status.vehicle);
  }, [status?.vehicle]);
  ...
  useLiveMessages(() => {
    ...
    liveThrottleUntilRef.current = now + LIVE_THROTTLE_MS;
    setLiveTick((t) => t + 1);
  });
  ...
  return (
    ...
    >
            ...
            <Table striped highlightOnHover>
              ...
              <Table.Tbody>
                {uploadErrors.map((e, i) => (
                  <Table.Tr key={i}>
                    <Table.Td style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>{String(e.viewId ?? '—')}</Table.Td>
                    <Table.Td style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>{e.path}</Table.Td>
      ...
      <Modal
        opened={opened}
        onClose={close}
        title={editing?.id && views?.some((x) => x.id === editing.id) ? 'Edit view' : 'New view'}
        size="lg"
      >
        {editing && (
          <Stack gap="sm">
            ...
            {editing.viewType === 'sync' ? (
              <>
                ...
                <Paper withBorder p="sm" radius="sm" style={{ background: 'var(--bg-elevated)', borderColor: 'var(--border)' }}>
...
function StandardAnalyticsViewCard({
      ...
      try {
        ...
        if (view.viewType === 'min' || view.viewType === 'max') {
          ...
          const r = await apiJson('/api/analytics/series', {
            method: 'POST',
            body: JSON.stringify({ ...bodyBase, array_index: arrIdx, limit: 1 }),
          });
          setStat(null);
          const pts = r.points || [];
          ...
          const r = await apiJson('/api/analytics/series', {
            method: 'POST',
            body: JSON.stringify({ ...bodyBase, array_index: arrIdx, limit: 5000 }),
          });
          setStat(null);
          setSeries(r.points || []);
        }
  ...
  return (
    <Paper withBorder p="md" radius="md" style={{ minWidth: 0, maxWidth: '100%' }}>
      <Group justify="space-between" align="flex-start" wrap="nowrap" mb="xs">
        <Box style={{ minWidth: 0 }}>
          <Text fw={600} size="sm" truncate style={{ color: 'var(--text)' }}>
        ...
        <Group gap={4} wrap="nowrap">
          {viewCount > 1 && (
            <>
              <ActionIcon variant="default" size="sm" disabled={viewIndex <= 0} onClick={onMoveUp} title="Move up">
                <ChevronUp size={14} />
              </ActionIcon>
              <ActionIcon
                variant="default"
...
function SyncAnalyticsViewCard({ view, liveTick, onEdit, onDelete, viewIndex, viewCount, onMoveUp, onMoveDown }) {
              ...
              <Table.Tbody>
                {mergedRows.length === 0 ? (
                  ...
                  mergedRows.map((row, i) => (
                    <Table.Tr key={i}>
                      <Table.Td style={{ fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap' }}>
                        {String(row.t ?? '').slice(0, 28)}
                      </Table.Td>
                      <Table.Td style={{ fontFamily: 'var(--font-mono)' }}>
