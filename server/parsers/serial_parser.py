...
class SerialParser(Parser, Logger):
    def __init__(self, packet_queue, stop_event, port, baudrate=125000, log_queue=None):
    ...
    def run(self):
        ...
        try:
            ...
            if baudrate in baudrate_codes:
                code = baudrate_codes[baudrate]
                cmd = f'S{code}\\r'.encode()
                print(f"[{self.__class__.__name__}] Sending Baud Config: S{code}")
                ser.write(cmd)
                time.sleep(0.1)
                ser.read_all()

            print(f"[{self.__class__.__name__}] Sending Open Command: O")
            ser.write(b'O\\r')
            time.sleep(0.1)
            ser.read_all()
                    ...
                    if raw_line:
                        try:
                            ...
                            if line_str and (line_str.startswith('t') or line_str.startswith('T')):
                                ...
                                self.packet_queue.put(line_str)
                        except Exception as decode_err:
                            print(f"[{self.__class__.__name__}] Decode Error: {decode_err}")
